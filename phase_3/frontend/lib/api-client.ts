...
async function getAuthHeader(): Promise<string> {
  ...
  // Retry once on transient failures (e.g. token bridge cold start)
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(body.error || `Token bridge error (${res.status})`);
      }
      ...
      // Don't retry auth failures — only transient ones
      if (lastError.message.includes("No active session")) {
        throw lastError;
      }
      if (attempt === 0) {
...
export const api = {
  ...
  chat: {
     ...
     * No JWT auth — user_id is passed in the URL path
     * @param userId - User identifier from auth session
     * @param message - Message text (1-10000 chars)
    ...
    sendMessage: async (
      ...
      if (!res.ok) {
        const errorData = await res.json().catch(() => ({}));
        throw new Error(
          errorData.detail || `Chat request failed: ${res.status}`
        );
      }
